---
description: Complete reference for SpecFlow slash commands including /sf:pm, /sf:analyst, /sf:architect, /sf:security, /sf:dev, and more.
---
...
## MCP Tool Calls (No Slash Command)

SpecFlow also exposes MCP tools directly. These are the runtime interfaces used by PM and agents behind slash commands.

| Tool | Purpose | Typical Call |
|------|---------|--------------|
| `specflow_state` | Workflow/session state transitions and recovery | `specflow_state("resume")` |
| `specflow_context` | Phase-scoped persona/expertise/artifact assembly | `specflow_context("architect")` |
| `specflow_validate` | Artifact validation checks for a phase | `specflow_validate("dev-story")` |
| `specflow_codebase` | Code intelligence queries (scan/symbols/patterns/dependencies) | `specflow_codebase("symbols", { path: "src/" })` |
| `specflow_impact` | Blast-radius analysis for symbol changes | `specflow_impact("handleState", 2)` |

Use direct tool calls when testing orchestration behavior, IDE MCP integration, or non-slash automation paths.
...
**Phase naming note:** the MCP workflow phase is `dev-story` (not `dev`), even when using `/sf:dev` in chat UIs.
...
## PMO Reporting via PM

Reporting is routed through `/sf:pm` using report-capable skills.

Examples:

```bash
/sf:pm "generate an executive status report for this feature"
/sf:pm "run a project health check"
/sf:pm "create a RAID/risk report"
```

See [PMO Reporting Skills](/reporting-skills) for output files and skill behavior.
