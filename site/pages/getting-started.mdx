---
description: Install SpecFlow and build your first feature with AI-powered security analysis, cost estimation, and test planning in minutes.
---
...
This creates directories in your project:
...
| `.mcp.json` | MCP server registration for direct tool calls |

The init process detects your project type (Node.js, Python, Go, etc.) and configures the workspace accordingly.

### Optional: Direct MCP Workflow

If you are testing orchestration directly (instead of using slash commands), the MCP loop is:

```text
specflow_state("resume")
specflow_state("start", { feature: "my-feature", description: "..." })
specflow_context("triage")
...write output artifact...
specflow_validate("triage")
```
...
The Test Engineering Analyst creates Gherkin scenarios that become your acceptance criteria:
